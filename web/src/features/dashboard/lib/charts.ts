...
export function getDashboardChartColors(domainLength: number): string[] {
  ...
  if (!scheme) {
    return ['#5B8FF9', '#5AD8A6', '#F6BD16', '#E8684A', '#6DC8EC']
  }
...
export function processUserChartData(
  ...
  const userColorRange = getDashboardChartColors(Math.max(limit, 2))
  ...
  const registrationValues = [...registrationMap.entries()]
  ...
  const paymentValues = [...paymentMap.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([Time, item]) => ({
      Time,
      Count: item.Count,
